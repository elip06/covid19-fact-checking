<template>
  <v-main class="wrapper" fill-height>
    <v-card id="highlighted-text-container" flat class="ml-2 mt-1" max-width="694" fill-height>
      <span class="pb-0">The sentences, containing potentially dubious claims, are highlighted accordingly:</span>
      <v-timeline
        class="ml-n15 pt-1"
        dense>
    <v-timeline-item class="pb-1" small color="red lighten-2">False</v-timeline-item>
    <v-timeline-item class="pb-1" small color="deep-orange lighten-2">
      Mostly False
    </v-timeline-item>
    <v-timeline-item class="pb-1" small color="yellow lighten-2">Mixture</v-timeline-item>
    <v-timeline-item class="pb-1" small color="lime lighten-2">MostlyTrue</v-timeline-item>
    <v-timeline-item class="pb-1" small color="green lighten-2">True</v-timeline-item>
    <v-timeline-item class="pb-3" small color="cyan lighten-2">Missing info/Unclear</v-timeline-item>
  </v-timeline>
...
export default {
  ...
  data() {
    return {
      ...
      falseValues: ['false', 'four pinocchios', 'inaccurate', 'miscaptioned', 'misattributed', 'scam'],
      colorMapping: {
        1: 'red lighten-2',
  ...
  methods: {
    ...
    determineRating: function(textualRating) {
      if (this.mostlyTrueValues.find(v => textualRating.includes(v))) {
        return 4;
      } else if (this.mostlyFalseValues.find(v => textualRating.includes(v))) {
        return 2;
